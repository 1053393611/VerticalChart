...
- (void)awakeFromNib{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refreshEveryResult:) name:@"refreshEveryResult" object:nil];
}
...
- (void)aboutControlView {
    ...
    [openTextView resignFirstResponder];
    ...
    for (int i = 0; i < 6; i++) {
        UILabel *label = [self viewWithTag:801 + i];
        label.textColor = [UIColor colorWithQuick:62 green:131 blue:148];
        label.text = @"0";
    }
}
...
#pragma mark - 计算每个数字的开牌结果
- (void)refreshEveryResult:(NSNotification *)noti {
    [self setEveryResult];
}
...
- (void)setEveryResult {
    for (int i = 0; i < 6; i++) {
        UILabel *label = [self viewWithTag:801 + i];
        float all = [self getEveryResult:i + 1];
        if (all >= 0) {
            label.textColor = [UIColor colorWithQuick:62 green:131 blue:148];
        }else {
            label.textColor = [UIColor colorWithQuick:255 green:38 blue:0];
        }
        label.text = [self getStringWithNumber:all];
    }
}


- (float)getEveryResult:(NSInteger)resultNumber{
    NSMutableArray *darray = [[DataManager defaultManager] getData];
    float all = 0;
    for (int i = 1; i < cellMax; i++) {
        NSMutableDictionary *dic = darray[i];
        float thisAll = 0;
        if ([[dic objectForKey:@"input"] isEqualToString:@""]) {
            NSString *update = [dic objectForKey:@"update"];
