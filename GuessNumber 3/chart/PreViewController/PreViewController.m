...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView.tag == 102) {
        ...
        for (int i = 1; i < cellMax; i ++) {
            NSString *str = [detailArray[i] objectForKey:@"input"];
            if ([str containsString:@"\n"]) {
                NSString *text = [[DataManager defaultManager] textForString:str];
                [_textArray replaceObjectAtIndex:i withObject:text];
            }else{
                [_textArray replaceObjectAtIndex:i withObject:str];
            }
        }
        
        for (int i = 1; i < cellMax; i++) {
            NSString *input = _textArray[i];
            CGFloat inputHeight = [self heightForText:input];
            if (inputHeight > cellHeight) {
                [self->heightArray replaceObjectAtIndex:i withObject:@(inputHeight)];
            }else {
...
- (CGFloat)heightForText:(NSString *)str{
    ...
    CGFloat width = HBScreenWidth * 169 / 560.f + 20;
...
//    NSLog(@"双击%ld",index);
    ...
    vc.reloadTheVC = ^{
        ...
        for (int i = 1; i < cellMax; i ++) {
            NSString *str = [self->detailArray[i] objectForKey:@"input"];
            if ([str containsString:@"\n"]) {
                NSString *text = [[DataManager defaultManager] textForString:str];
                [self.textArray replaceObjectAtIndex:i withObject:text];
            }else{
                [self.textArray replaceObjectAtIndex:i withObject:str];
            }
        }
        for (int i = 1; i < cellMax; i++) {
            NSString *input = self.textArray[i];
            CGFloat inputHeight = [self heightForText:input];
            if (inputHeight > cellHeight) {
                [self->heightArray replaceObjectAtIndex:i withObject:@(inputHeight)];
            }else {
